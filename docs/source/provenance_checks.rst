...
6. ``ERROR``/``INVALID-LOCK-HASH`` - the lockfile hash does not correspond to the hash computed, this generally mean that requirements were adjusted and the lockfile no longer corresponds to the requirements file used
