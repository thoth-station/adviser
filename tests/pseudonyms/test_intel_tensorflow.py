...
"""Test intel-tensorflow pseudonym."""
...
from thoth.adviser.pseudonyms import IntelTensorFlowPseudonym
...
class TestIntelTensorFlowPseudonym(AdviserUnitTestCase):
    """Test recommending intel-tensorflow where tensorflow is a dependency."""

    UNIT_TESTED = IntelTensorFlowPseudonym
    ...
    @pytest.mark.parametrize(
        ...
        [
            (RecommendationType.LATEST, None, None),  # Do not include for LATEST
            (RecommendationType.TESTING, None, "10.1"),  # Do not include when CUDA is used.
    ...
    def test_run_pseudonym(self, context: Context) -> None:
        ...
        context.graph.should_receive("get_solved_python_package_versions_all").with_args(
            ...
            [
                ("intel-tensorflow", "1.0.0", "https://pypi.org/simple"),
                ("intel-tensorflow", "2.0.0", "https://pypi.org/simple"),
            ]
        ).once()
        ...
        with unit.assigned_context(context):
            ...
            assert list(result) == [("intel-tensorflow", package_version.locked_version, package_version.index.url)]
